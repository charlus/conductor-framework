...
- **Deterministic enforcement** → `templates/.agents/hooks/` (`lib.sh` + `pre-commit` for the TDD + Eval *presence* gates, the Goodhart boundary and protected paths; `pre-push` for Verify + Eval *run* gates and the pinned official `pre-commit` fingerprint), wired by `conductor install-hooks`. The hooks load the *committed* `lib.sh` and fail closed without it. Opt-in Claude Code `PreToolUse` hooks: `pretooluse-no-bypass.sh` and `pretooluse-fact-gate.sh` (measured by `test/evals/fact-gate-eval.mjs`). `test/hooks-registry-drift.test.js` keeps hooks, README and `install-hooks` in step. `conductor upgrade` commits its own protected-path writes (`src/commands/upgrade.js`, `commitUpgrade`). Design + review record: `docs/roadmap/Peer-Framework-Harvest.md`. The ship-contract (Eval-Driven Law + `architecture-checklist`, judge-rubric calibration): `docs/roadmap/Eval-Driven-Law.md`.

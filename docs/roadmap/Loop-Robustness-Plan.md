...
>
> **Remaining:** **P1.4** merge-conflict/resume discipline (the resume half), **P2** (self-improvement loop, architecture-checklist contract, multi-engine parity), higher-concurrency mileage (concurrency ≥ 3, larger/heterogeneous queues), and the leftover testing cases (fake-maker forgets-to-commit; done-claimed-but-no-commit re-prompt).

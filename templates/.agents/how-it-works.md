...
Tests verify the **deterministic** parts; **evals** verify the **non-deterministic** LLM-output surface of the app you're building. If a feature calls an LLM provider, a passing unit test is not enough — it needs an evalset. The `pre-commit` hook enforces it deterministically: provider-calling code staged without an eval (an `evals/` file or `*.eval.*`) is blocked, exactly like the Test-Driven Law blocks impl-without-test. The two gates are independent; waiving one (`CONDUCTOR_NO_EVAL="reason"`) never skips the other. The *how* — three grading modes (rubric/LM-judge, property/assertion, reference) — lives in the on-demand `skills/writing-evals/` skill, loaded by Build when a task touches LLM-feature code. It is **not** an always-on rule: evals matter only to LLM-feature projects, so the enforcement is the (silent-when-irrelevant) hook, not static-context prose. Design: `docs/roadmap/Eval-Driven-Law.md`.
